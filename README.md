...
## Содержание
<b><h2><a href="#g0">Раздел для пользователя</a></h2></b>
1. <a href="#g1">Общее описание задачи</a>
1. <a href="#g2">Описание данных</a>
1. <a href="#g3">Установка пакета, получение данных, использование Poetry</a>
...
1. <a href="#g10">Один полный пример скрипта</a>
<b><h2><a href="#g00">Раздел для разработчика</a></h2></b>
11. <a href="#g11">Реализованные тесты</a>
12. <a href="#g12">Форматирование кода в проекте</a>
13. <a href="#g13">Аннотация типов в коде</a>
14. <a href="#g14">Проверка всего подготовленного пакета одной командой</a>
15. <a href="#g15">CI/CD с использованием функционала github</a>

***
<h2 id="g0">Раздел для пользователя</h2>

<h3 id="g1">Общее описание задачи</h3>
...
При разработке представленного пакет был использован менеджер зависимостей <b>Poetry</b>. Использованные библиотеки разбиты на две группы:

* библиотеки необходимые для нрмального функционирования пакета;
* библиотеки используемые разработчиками для улучшения читабельности кода и его проверки на функционирование.

Данные, на которых осуществляется обучение модели, расположены по адресу: https://www.kaggle.com/competitions/forest-cover-type-prediction/data

***
<h3 id="g2">Описание данных</h3>
...
***
<h3 id="g3">Установка пакета, получение данных, использование Poetry</h3>
...
1. Как уже указывалось выше данные можно получить из соответствующего соревнования на kaggle. Другим вариантом получения данных является клонирование отдельного репозитория в котором сохранены те самые данные на случай если на площадке kaggle их по какой-то причине поменяют. Выполнить это можно с помощью команды <b>git clone https://github.com/DIVIGL1/data.git</b>

#### Рекомендация по размещению данных
Рекомендуется данные расположить в директории <b>data</b>, которую нужно расположить внутри основного репозитория. Это позволит не указывать в командной строке каждый раз место расположения данных, так как пакет по умолчанию подставляет указанный путь (data) к файлам train.csv и test.csv и использует для сохранения софрмированных моделей и/или прогнозов.

Как следствие, структура каталогов будет/может выглядеть примерно следующим образом:

![DirTree](https://github.com/DIVIGL1/RSS-Evaluation-selection/blob/main/picts/DirTree.PNG?raw=true)

#### Автоматизация установки
Для упрощения процесса установки можно создать файлы со скриптами, как это показано ниже (они же входят в состав репозитория: Install.m9.bat и Install.m9.sh).

* Пакетный файл Windows (Install.m9.bat):
...
***
<h3 id="g4">Реализация командной строки</h3>
...
***
<h3 id="g5">Сведения об использованных моделях машинного обучения</h3>
...
***
<h3 id="g6">Сведения о реализованных подходах подбора и генерации признаков (feature selection)</h3>

Сточки зрения обработки данных предложено четыре варианта, каждый из которых выбирается через параметр командной строки:
...
***
<h3 id="g7">Метрики использованные для оценки моделей</h3>
...
***
<h3 id="g8">Применение MLFlow для сохранения результатов работы пакета</h3>
...
***
<h3 id="g9">Обычный и nested варианты Cross-validation и подбор гиперпараметров</h3>
...
Для раализации сказанного выше введён параметр командной строки --nested-cv, который по умолчанию равен <b>False</b> и в этом случае используется функциональность <b>обычного варианты Cross-validation</b>. В случае если этот параметр установлен в значение <b>True</b>, то используется <b>nested Cross-validation</b> с подбором гиперпараметров (в этом случае все заданные вручную параметры игнорируются).
...
1. используя лучшую модель, формируется предсказание на всём наборе данных и для сравнения рассчитываются те же выбранные метрики (отображается на экране);
1. используя лучшую модель, производится расчет по принципам задания №7 (значения вычесленных метрик выводятся на экран, а также производится сохранение результатов в MLFlow.
...
***
<h3 id="g10">Один полный пример скрипта</h3>
Для полноценной демонстрации функциональности пакета приводим здесь вариант одного скрипта:
...
***
<h2 id="g00">Раздел для разработчика</h2>
#### Важно!

Обратите внимание, что предложенная выше в скриптах установка пакета использует параметр <b>--no-dev</b> что исключает установку компонент необходимых для разработчика. Если вы планируете продолжить работу над проектом, то вам необходимо либо исключить этот параметр, либо дополнительно провести установку, но уже с параметром <b>--dev</b>.

Так же стоит отметить, что:

1. обязательным является ведение файла .gitignore, который в настоящее время исключает сохранение:

* данных на girhub;
* компилированных файлов;
* фалйов настройки VSCode;
* результатов работы MLFlow;
* временных файлов black, mypy, nox;
* папок виртуальных сред Python (если они начинаются с .env)
* и т.д. (подробнее в файле .gitignore)

2. Для разработчика важно обеспечивать возможнсть проверки результатов другими участниками проекта. Поэтому, везде где это возможно, должны быть применены параметры типа pandom_state и seed, что позволяет сохраненить повторяемость тестов. В настояще время, по умолчанию эти параметры принимают значение 42 ("ответ на главный вопрос жизни, Вселенной и всего такого").

***

<h3 id="g11">Реализованные тесты</h3>

Для обеспечения проверки результатов функционирования кода после внесения изменений необходимо проводить проверку его на тестовых примерах. В данном случае это выполнено на библиотеке pytest c реализацией следующих проверок:

* на фиктивный параметр;
* на граничные значения random_state;
* на запуск с отсутствующими данными;
* на возможность сохранения модели;
* на возможность сохранения прогноза;
* правильности прогноза на тестовом примере.

Стоит отметить, что кроме этих тестов все параметры, применяемые в командной строке, контролируются на правильность их ввода с помощью библиотеки Click. Так, например, <b>--c-param</b> проверяется не только на тип (float), но и на то чтобы быть положительным, а <b>--algorithm</b> на соответствие значения возможному из списка ("auto", "ball_tree", "kd_tree", "brute").
...
Для цели полноценного тестирования используются искусственные данные (сгенерированные в тестовом скрипте), результат формирования моделии и её прогноза заранее известен. Во время тестиа на этих данных обучается модель, а её прогноз сверяется с известным.

Во время тестирования и подготовки искусственных данных используется изолированная файловая системе: файлы создаются во временном каталоге и после отработки тестов автоматически удаляются.

#### Важно!
Для выполнения тестов, необходимо, чтобы установка пакета происходила:
* либо без ключа: --no-dev
* либо дополнительно были доустановлены компоненты для разработчика командой: poetry install --dev

***
<h3 id="g12">Форматирование кода в проекте</h3>
...
После обработки кода утилитой black, потребовалось дополнительно подкорректировать полученный результат, чтобы избавиться от предупреждений, выданных flake8. Большая часть замечаний связана с длинной строки:

![flake8](https://github.com/DIVIGL1/RSS-Evaluation-selection/blob/main/picts/flake8.PNG?raw=true)

Замечания от flake8 были устранены введением исключений E203, E501 и W503, для чего создан конфигурационный файл .flake8 и внесены правки в файле настроек VSCode (последних нет в репозитории):

![Afterflake8](https://github.com/DIVIGL1/RSS-Evaluation-selection/blob/main/picts/Afterflake8.PNG?raw=true)

#### Важно!
Для применения данных команд необходимо, чтобы установка пакета происходила:
* либо без ключа: --no-dev
* либо дополнительно были доустановлены компоненты для разработчика командой: poetry install --dev

***
<h3 id="g13">Аннотация типов в коде</h3>
Все реализованные в коде методы снабжены аннотациями типов и правильно используются во всем коде. Проверка осуществлена с помощью mypy и пройдена успешно. Это видно на следующем скрине:

![mypy](https://github.com/DIVIGL1/RSS-Evaluation-selection/blob/main/picts/mypy.PNG?raw=true)

#### Важно!
Для применения данной команды необходимо, чтобы установка пакета происходила:
* либо без ключа: --no-dev
* либо дополнительно были доустановлены компоненты для разработчика командой: poetry install --dev

***
<h3 id="g14">Проверка всего пакета одной командой</h3>
...
***
<h3 id="g15">CI/CD с использованием функционала github</h3>

Описание действий (actions) представлено на следующем снимке:

![ci/cd](https://github.com/DIVIGL1/RSS-Evaluation-selection/blob/main/picts/ci_cd.png?raw=true)

А ниже результаты работы на github:

![ci_cd_nox](https://github.com/DIVIGL1/RSS-Evaluation-selection/blob/main/picts/ci_cd_nox.PNG?raw=true)
